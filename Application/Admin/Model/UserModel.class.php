...
class UserModel extends Model
{
    protected $_validate = array(
        ...
        array('u_username','/^[a-zA-z][a-zA-Z0-9_]{2,9}$/','用户名格式不正确！',0,'regex',3), // 正则验证用户名
        array('u_password','/((?=.*\d)(?=.*\D)|(?=.*[a-zA-Z])(?=.*[^a-zA-Z]))^.{8,16}$/','密码格式不正确',0,'regex',3), // 验证密码格式 长度8~16位 数字、字母、字符至少包含两种。
    );
